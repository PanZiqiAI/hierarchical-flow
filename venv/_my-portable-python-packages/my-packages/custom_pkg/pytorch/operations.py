    ...
    def _get_lmd(_lmd):
        ...
        def __get_single_lmd(_l):
            if isinstance(_l, float):
                _l = wrap_hp(hyper_param=_l)(lambda x: x)
            else:
                if not hasattr(_l, "hyper_param"):
                    _l = wrap_hp(hyper_param=1.0)(_l)
            # Return
            return _l
...
def nansum(x, dim=0, keepdim=False):
    valid = (~torch.isnan(x)) * (~torch.isinf(x))
    x = torch.where(valid, x, torch.zeros_like(x))
    return torch.sum(x, dim=dim, keepdim=keepdim)


def nanmean(x, dim=0, keepdim=False):
    valid = (~torch.isnan(x)) * (~torch.isinf(x))
    x = torch.where(valid, x, torch.zeros_like(x))
    return torch.sum(x, dim=dim, keepdim=keepdim) / torch.sum(valid, dim=dim, keepdim=keepdim)
