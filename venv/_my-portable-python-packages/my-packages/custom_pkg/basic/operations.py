...
class BatchSlicerInt(object):
    ...
    def __init__(self, x, batch_size, ret_mode='batch_size'):
        assert ret_mode in ['batch_size', 'values', 'range']
        # Config
        self._batch_size = batch_size
        if ret_mode == 'batch_size': self._lmd_ret = lambda _count, _batch_size: _batch_size
        elif ret_mode == 'values': self._lmd_ret = lambda _count, _batch_size: range(_count, _count+_batch_size)
        else: self._lmd_ret = lambda _count, _batch_size: (_count, _count+_batch_size)
        # Members
        self._x = x
        self._counts = 0
