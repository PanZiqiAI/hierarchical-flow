...
class Coupling(InvertibleModule):
    """
    Coupling: (nc, h, w) -> (nc, h, w).
    """
    def __init__(self, input_nc, hidden_nc, mode):
        super(Coupling, self).__init__()
        assert mode in ['additive', 'affine']
        # --------------------------------------------------------------------------------------------------------------
        # Architecture.
        # --------------------------------------------------------------------------------------------------------------
        ...
        self._nn.append(ReLU())
        self._nn.append(Conv2d(hidden_nc, input_nc if mode == 'affine' else input_nc//2, kernel_size=3, stride=1, padding=1))


class AdditiveCoupling(Coupling):
    """ Additive coupling. """
    def __init__(self, input_nc, hidden_nc):
        super(AdditiveCoupling, self).__init__(input_nc, hidden_nc, mode='additive')
        # --------------------------------------------------------------------------------------------------------------
        # Gradients buffer
        # --------------------------------------------------------------------------------------------------------------
    ...
    def forward(self, x, linearize=False, compute_logdet=False):
        ...
        # --------------------------------------------------------------------------------------------------------------
        if compute_logdet:
            logdet = Logdet(total_logdet=torch.zeros(len(x), dtype=x.dtype, device=x.device), nc=x.size(1), img_size=x.size(2))
            # Return
    ...
    def linearized_transpose(self, eps):
        """ Given x_a -> output_b as output_b = x_b + t, we have that
            output_b' = t'.
        """
        # 1. Split.
        ...
        # (1) NN.
        output_nn = eps_b
        for module in self._nn[::-1]: output_nn = module.linearized_transpose(output_nn)
        # (2) Additive.
        output_a = eps_a + output_nn
        # 3. Merge.
        output = torch.cat([output_a, eps_b], dim=1)
        # Return
        return output
...
class AffineCoupling(Coupling):
    """ Affine coupling. """
    def __init__(self, input_nc, hidden_nc):
        super(AffineCoupling, self).__init__(input_nc, hidden_nc, mode='affine')
        # --------------------------------------------------------------------------------------------------------------
        # Gradients buffer
        # --------------------------------------------------------------------------------------------------------------
...
class FlowUpsample(InvertibleModule):
    ...
    """
    def __init__(self, input_nc, hidden_nc, coupling_mode, n_couplings, upsample=2):
        super(FlowUpsample, self).__init__()
        assert input_nc % 2 == 0 and input_nc % upsample**2 == 0
        assert coupling_mode in ['additive', 'affine']
        # --------------------------------------------------------------------------------------------------------------
        # Architecture.
        # --------------------------------------------------------------------------------------------------------------
        # 1. Pre-conv + couplings + convs.
        self._pre_conv = InvConv2d1x1Fixed(input_nc)
        self._couplings = nn.ModuleList([(AffineCoupling if coupling_mode == 'affine' else AdditiveCoupling)(input_nc, hidden_nc) for _ in range(n_couplings)])
    ...
    def eval_debug(self, x, sv_pred, jacob_size=16):
        ...
        with torch.no_grad():
            ...
            """ Com. """
            com_jacob_err = normalized_mean_absolute_err(pred=com_jacob, gt=flow_jacob*sv_pred.reshape(len(sv_pred), 1, -1))
            com_jacob_lt_err = normalized_mean_absolute_err(pred=com_jacob_t, gt=flow_jacob_t*sv_pred.reshape(len(sv_pred), -1, 1))
            """ Logdet. """
