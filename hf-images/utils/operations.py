...
def update_ema(stale, new, beta=0.99):
    return stale * beta + new * (1.0 - beta)
...
def normalized_mean_absolute_err(pred, gt):
    """ nMAE. The first axis should be batched. """
...
def tpr95(in_distances, out_distances):
    """
    False Positive Rate (FPR) when TPR == 95%.
    """
    """ Get delta. """
    reversed_in_distances = torch.sort(in_distances, descending=True).values
    upper_num = int(0.9505 * len(reversed_in_distances))
    lower_num = int(0.9495 * len(reversed_in_distances))
    if upper_num == lower_num:
        delta_list = [reversed_in_distances[upper_num]]
    else:
        delta_list = reversed_in_distances[lower_num:upper_num]
    """ Compute metric. """
    result, counter = 0.0, 0
    for delta in delta_list:
        fpr = torch.sum(out_distances >= delta).item() * 1.0 / len(out_distances)
        result += fpr
        counter += 1
...
def fast_approx_logsn(x, output, x_t, output_t, sn_power=3):
    ...
    # 3. Get results.
    logsn = ((v.reshape(v.size(0), -1)**2).sum(dim=1) + 1e-8).log() * 0.5
    # Return
    return logsn
