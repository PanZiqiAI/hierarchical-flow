...
class ConfigTrain(CanonicalConfigPyTorch):
    ...
    def _add_root_args(self):
        ...
        self.parser.add_argument("--dataset",           type=str,   default='circle')

    def _add_tree_args(self, args_dict):
        ################################################################################################################
        # Datasets.
        ################################################################################################################
        if args_dict['dataset'] == 'circle':
            self.parser.set('img_nc', 2)
            self.parser.add_argument("--dataset_n_samples",     type=int,   default=10000)
        ...
        self.parser.add_argument("--tracking_u_beta",           type=float, default=0.99)
        # SV Predictor.
        ...
        if args_dict['dataset'] == 'circle':
            self.parser.add_argument("--eval_vis_n_grids",          type=int,   default=10)
            self.parser.add_argument("--eval_vis_n_samples_curve",  type=int,   default=100)
        # Jacobian.
